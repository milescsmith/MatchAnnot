...
class Aligner(object):
    ...
    def __init__(self, match=1, mismatch=-3, gap=-1, maxRef=1, maxRead=1):
        ...
        self._matrix = np.zeros(
            shape=(maxRead + 1, maxRef + 1), dtype=int
        )  # read is X axis, ref is Y
    ...
    def fillMatrix(self):
        ...
        if curRows < rows or curCols < cols:  # we're gonna need a bigger matrix
            ...
            self._matrix = np.zeros(
                shape=(max(curRows, rows), max(curCols, cols)), dtype=int
            )  # dtype=int is *not* the default!
    ...
    def alignmentStrings(self, pos=None):
        ...
        while row > 0 and col > 0:  # loop runs backwards, we'll reverse at the end
            ...
            m_mch = mx[row - 1][col - 1]
            # scores for match, insert, delete
            m_ins = mx[row][col - 1]
            m_del = mx[row - 1][col]
